...
NC_indices = idxs_first[mask_first]
NC_values  = ratio_first[mask_first]
FC_indices = idxs_first[~mask_first]
FC_values  = ratio_first[~mask_first]

NC_order = np.argsort(NC_indices)
NC_indices = NC_indices[NC_order]
NC_values  = NC_values[NC_order]

FC_order = np.argsort(FC_indices)
FC_indices = FC_indices[FC_order]
FC_values  = FC_values[FC_order]
...
legend_ge1 = Line2D([0],[0], marker='o', linestyle='none', markerfacecolor=rep_high_color, markeredgecolor='k', markersize=7, label=f"ratio ≥ 1 ({len(FC_indices)})")

legend_lt1 = Line2D([0],[0], marker='o', linestyle='none', markerfacecolor=rep_low_color, markeredgecolor='k', markersize=7, label=f"ratio < 1 ({len(NC_indices)})")

plt.legend(handles=[legend_ge1, legend_lt1], loc="best")

plt.savefig(args.output_path + "/results/ratio_scatter.png", dpi=300)

print(f"{len(NC_indices)} unique events with ratio < 1")

'''
print("indices & ratio values (first occurrence, sorted by index):")
for i, (idx, val) in enumerate(zip(NC_indices, NC_values)):
  print(f"{i:4d}: index={idx}, ratio={val}")
'''

np.savez(
  args.output_path + "/results/FC_result.npz",
  NC_idxs=NC_indices, NC_vals=NC_values,
  FC_idxs=FC_indices, FC_vals=FC_values
)
