...
parser = argparse.ArgumentParser(description="Evaluation script for electron direction regression")
parser.add_argument("run_dir", type=str, help="Path to electron direction regression run directory")
...
# Electron events
direction_regression_output_e = reg.WatChMaLDirectionRegression(regression_run_dir, "Electron Regression", test_event_directions, test_idxs,)


fig, ax = direction_regression_output_e.plot_training_progression()
ax.set_yscale('log')
plt.tight_layout()
plt.savefig(results_dir + "Loss.png", bbox_inches='tight')

with open(results_dir + "resolution.txt", "w") as f:
  direction_resolution = np.quantile(direction_regression_output_e.direction_errors, 0.68)
...
fig, ax = reg.plot_resolution_profile([direction_regression_output_e], 'direction_errors', E_binning, x_label="True particle energy [MeV]", y_label="Direction resolution [°]", y_lim=(0,15), errors=True, x_errors=False)
plt.tight_layout()
plt.savefig(results_dir + "AngleResolution-E.png", bbox_inches='tight')


fig, ax = reg.plot_resolution_profile([direction_regression_output_e], 'direction_errors', towall_binning, x_label="Distance to detector wall in particle direction [cm]", y_label="Direction resolution [°]", y_lim=(0,15), errors=True, x_errors=False)
plt.tight_layout()
